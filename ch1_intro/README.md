...
- [1章 イントロダクション](#1章-イントロダクション)
  ...
  - [1.4 JavaScriptの基本](#14-javascriptの基本)
    ...
    - [1.4.7.1 module.exportsとrequire()](#1471-moduleexportsとrequire)
      - [1.4.7.2 __filenameと__dirname](#1472-__filenameと__dirname)
      - [1.4.7.3 strictモード](#1473-strictモード)
...
```
module.exportsのショートカットとして、exportsだけでもかける

またrequire()の引数には、ディレクトリも指定可能。指定したディレクトリ内にindex.jsがあれば、そのファイルがモジュールとしてロードされる。そのためこの仕組みを使って、実装をディレクトリ内の複数ファイルに分割しつつ、index.jsでまとめて公開することで、使用する側は1つ尾モジュールとして利用できる。(賢いね)
[例:cjs-math2/](cjs-math2/)

#### 1.4.7.2 __filenameと__dirname

module.exports、require意義にも__filename, __dirnameという変数がモジュールスコープに割り当てられてる。それぞれ、ファイル名、ディレクトリ名の絶対パスが割り当てられてる。

[cjs-filename-dirname.js](cjs-filename-dirname.js)

```
> require('./cjs-filename-dirname')
{
  __filename: 'C:\\Users\\<username>\\Desktop\\Node_JS_HandsOn\\ch1_intro\\cjs-filename-dirname.js',
  __dirname: 'C:\\Users\\<username>\\Desktop\\Node_JS_HandsOn\\ch1_intro'
}
```

#### 1.4.7.3 strictモード
CommonJSモジュールのファイルの先頭に'use strict'と記述すると、strictモードになる。
次のような特徴がある。
* 無駄な処理や、意図しない動きを引きおこしうる処理をエラーに
* 最適化を邪魔する機能を制限or無効にする
* 将来使われうる識別子を予約語として使えないようにする
  
あえてタイプミス
[cjs-non-strict-mode.js](cjs-non-strict-mode.js)
```
let mystring = 'いろは'
...
>node cjs-non-strict-mode.js 
にほへと
```

strictモード
[cjs-strict-mode.js](cjs-strict-mode.js)
```
'use strict'
let mystring = 'いろは'
...
>node cjs-strict-mode.js    
path\Node_JS_HandsOn\ch1_intro\cjs-strict-mode.js:5
mystrng = 'にほへと'
        ^
